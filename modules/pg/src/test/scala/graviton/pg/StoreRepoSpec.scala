...
object StoreRepoSpec extends ZIOSpecDefault {
  ...
  override def spec: ZSpec[TestEnvironment & Scope, Any] =
    suite("StoreRepo")(
      ...
      test("listActive continues fetching beyond the first page") {
        for {
          repo  <- ZIO.service[StoreRepo]
          _     <- ZIO.foreachDiscard(1 to 5)(i => repo.upsert(sampleRow(StoreStatus.Active, i + 10)))
          _     <- repo.upsert(sampleRow(StoreStatus.Retired, 99))
          cursor = graviton.db.Cursor.initial.copy(pageSize = 2L)
          rows  <- repo.listActive(Some(cursor)).runCollect
        } yield assertTrue(rows.length == 5, rows.forall(_.status == StoreStatus.Active))
      },
