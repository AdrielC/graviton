...
object BlobStoreRepoSpec extends ZIOSpecDefault:
    ...
    ZLayer.scoped {
      ZIO.acquireRelease {
        val c: PostgreSQLContainer[?] =
          new PostgreSQLContainer(DockerImageName.parse("postgres:16-alpine")).withInitScript("ddl.sql")
        c.start()
        ZIO.succeed(c)
      }(c => ZIO.succeed(c.stop()))
    }

  private val transactorLayer: ZLayer[PostgreSQLContainer[?], Nothing, Transactor] =
    ZLayer.fromZIO {
  ...
  private val repoLayer = containerLayer >>> transactorLayer >>> BlobStoreRepoLive.layer

  private def sampleRow: BlobStoreRow =
    val key   = Chunk.fromArray(Array.fill(32)(1.toByte))
    val bytes = Chunk.fromArray(Array[Byte](1,2,3))
    BlobStoreRow(key, "fs", bytes, "urn:test", bytes, None, StoreStatus.Active, 0L)
