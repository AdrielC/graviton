...
trait BlockRepo:
  def upsertBlock(key: BlockKey, size: PosLong, inline: Option[SmallBytes]): Task[Unit]
  ...
  def getHead(key: BlockKey): Task[Option[(PosLong, Boolean)]] =
    transact(xa) {
      ZIO.attempt {
        ...
        """.query[(Long, Boolean)].run().headOption
      }
    }
    ...
    transact(xa) {
      ZIO.attempt {
        sql"""
          ...
          VALUES (${key.algoId}, ${key.hash}, $storeKey, $uri, 'active', $len, $etag, $storageClass)
