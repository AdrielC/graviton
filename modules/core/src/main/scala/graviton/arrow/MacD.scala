...
object MacD:
  ...
  object Prim:
    ...
    final case class StopReport[Label <: String & Singleton](id: String) extends Prim[StopSignal, Report, NamedTuple.Empty]

  opaque type Stage[-I, +O] <: (State, I) => (State, O) = (State, I) => (State, O)

  object Stage:

    extension [I, O](stage: Stage[I, O])
      def run(state: State, input: I): (State, O) = stage(state, input)
    ...
    given bundle: ArrowBundle.Aux[Stage, Tuple2, Either] = new ArrowBundle[Stage]:
        ...
        def toLeft[A]: (State, A) => (State, Either[A, Nothing]) = 
          { (state, a) => (state, Left(a)) }

        def toRight[B]: (State, B) => (State, Either[Nothing, B]) = 
          { (state, b) => (state, Right(b)) }
        ...
        case p: Prim.ShortCircuit[l] =>
          Stage[Chunk[Frame], Either[StopSignal, Chunk[Frame]]] { (state, frames) =>
            if p.predicate(frames.asInstanceOf[Chunk[Frame]]) then
              ...
              (state, Right(frames.asInstanceOf[Chunk[Frame]]))
          }.asInstanceOf[Stage[I, O]]
  ...
  def stopReport[Label <: String & Singleton](id: String): FreeArrow.Aux[Prim, Tuple2, Either, StopSignal, Report, NamedTuple.Empty] =
  ...
  inline def drive[I, O, C <: NamedTuple.AnyNamedTuple](
