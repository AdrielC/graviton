...
final case class Range[A](start: A, end: A):
  ...
  def -(range: Range[A])(using discrete: Discrete[A], ord: Ordering[A]): Option[(Range[A], Option[Range[A]])] =
    if ord.lteq(range.start, start) then
      ...
      if ord.gt(range.start, end) then Some((this, None))
      else
        val r1 = Range(start, discrete.pred(range.start))
        val r2 = if ord.lt(range.end, end) then Some(Range(discrete.succ(range.end), end)) else None
        Some((r1, r2))

  def +(other: Range[A])(using ord: Ordering[A], discrete: Discrete[A]): (Range[A], Option[Range[A]]) =
    val (l, r) = if ord.lt(this.start, other.start) then (this, other) else (other, this)
  ...
  def toIterator(using discrete: Discrete[A], ord: Ordering[A]): Iterator[A] =
    val next: A => A = if ord.lt(start, end) then discrete.succ else discrete.pred
    Iterator.iterate(start)(next).takeWhile(a => !ord.equiv(a, end)).++(Iterator.single(end))
  ...
  def foreach(f: A => Unit)(using discrete: Discrete[A], ord: Ordering[A]): Unit =
    var i = start
