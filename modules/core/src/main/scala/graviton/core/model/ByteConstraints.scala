...
object ByteConstraints:
  ...
  type InlMinMax[
    Min <: Int | Long | Sized[? <: Int | Long, ? <: Min, ? <: Int | Long],
    Max <: Min | Sized[? <: Int | Long, ? <: Min, ? <: Max],
  ] =
    ((Min <<= Max), (Min >>= ZeroOf[Min]), (Max <<= MaxOf[Max]), (Max >>= Min))

  inline def allConstraints[
    Min <: Int | Long | Sized[? <: Int | Long, ? <: Min, ? <: Int | Long],
    Max <: Min | Sized[? <: Int | Long, ? <: Min, ? <: Max],
  ]: InlMinMax[Min, Max] =
    compiletime.summonAll[InlMinMax[Min, Max]]

  transparent sealed trait ByteSizeBounds[N <: Int | Long, Min <: N, Max <: N] extends ByteSize[N, Min, Max]:
  ...
  object UploadChunkSize
      extends ByteSizeBounds[Int, GlobalLimits.MIN_UPLOAD_CHUNK_SIZE_IN_BYTES, GlobalLimits.MAX_UPLOAD_CHUNK_SIZE_IN_BYTES]
