...
final case class BlockKey(hash: Hash, size: BlockSize)
...
object BlockKey:
  ...
  extension (key: BlockKey)
    def toBinaryKey: BinaryKey.CasKey.BlockKey = BinaryKey.CasKey.BlockKey(key.hash, key.size)
