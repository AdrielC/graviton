...
final class Scan[S <: Tuple, -I, +O](
  ...
  def map[O2](f: O => O2): Scan[S, I, O2] =
    Scan(initial)(
      (reg, i) => step(reg, i).map(f),
      reg => onComplete(reg).map(f)
    )
  ...
  def dimap[I2, O2](g: I2 => I)(f: O => O2): Scan[S, I2, O2] =
    Scan(initial)(
      (reg, i2) => step(reg, g(i2)).map(f),
      reg => onComplete(reg).map(f)
    )

  def andThen[S2 <: Tuple, O2](
      that: Scan[S2, O, O2]
  ): Scan[Tuple.Concat[S, S2], I, O2] =
    val init = initial ++ that.initial
    val sizeS = initial.productArity
    Scan[Tuple.Concat[S, S2], I, O2](init)(
      { (reg, i) =>
...
object Scan:
  def apply[S <: Tuple, I, O](initial: S)(
      step: (TypeRegister[S], I) => Chunk[O],
      onComplete: TypeRegister[S] => Chunk[O]
  ): Scan[S, I, O] =
