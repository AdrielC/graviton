 ...
 * `graviton.acl_entry.capabilities` (bigint). Each capability occupies one
...
enum Capability(val bit: Long):
  ...
  case ObservabilityRead extends Capability(1L << 3)
  case AuditRead         extends Capability(1L << 4)
  case AclAdmin          extends Capability(1L << 5)
