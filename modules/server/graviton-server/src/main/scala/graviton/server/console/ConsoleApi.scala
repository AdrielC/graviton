...
final class ConsoleApi(
  ...
  private def page(request: Request): UIO[Response] =
    ...
    yield htmlResponse(renderPage(listing, session.toString))).catchAll(errorResponse)
  ...
  private def renderPage(listing: CatalogListing, session: String): String =
    s"""<!doctype html>
       |<html lang="en">
       |<head>
       |  <meta charset="utf-8">
       ...
       |    </section>
       |    ${renderWorkspace(listing, session)}
       |  </div>
  ...
  private def renderWorkspace(listing: CatalogListing, session: String, notice: Option[String] = None): String =
    ...
    s"""<section id="workspace" class="workspace" data-folder="${escape(folderId)}" data-session="${escape(
       ...
       |      <button class="button" type="button" ${ConsoleDatastar.click(
        s"@get('${js(refreshUrl)}')"
      )} title="Refresh" aria-label="Refresh library">↻</button>
       ...
       |        <a class="tree-button$rootActive" ${navigationAttributes(rootLibrary, rootPage)}$rootCurrent><span>⌂</span> All files</a>
       |        ${listing.folders.map { folder =>
        val libraryUrl = consoleUrl("/console/library", Some(folder.id), Some(session))
        s"<a class=\"tree-button\" ${navigationAttributes(libraryUrl, consolePageUrl(Some(folder.id)))}><span>◇</span> ${escape(folder.name.value)}</a>"
      }.mkString}
       |      </nav>
  ...
  private def renderFolder(folder: CatalogFolder, parent: Option[CatalogFolderId], session: String): String =
    ...
    s"""<div class="entry folder">
       |  <div class="entry-name"><span class="entry-icon">◇</span><div class="name-stack"><a class="crumb name" ${navigationAttributes(
        openUrl,
       ...
       |  <div class="row-actions"><button class="icon-button danger" type="button" title="Remove empty folder" aria-label="Remove ${escape(
        folder.name.value
      )}" ${ConsoleDatastar.click(s"if (confirm('Remove this empty folder?')) @delete('${js(deleteUrl)}')")}>×</button></div>
       |</div>""".stripMargin
  ...
  private def renderFile(file: CatalogFile, folder: Option[CatalogFolderId], session: String): String =
    ...
    s"""<div class="entry">
       ...
       |  <div class="row-actions"><a class="icon-button" href="${escape(download)}" download="${escape(
        ...
        file.name.value
      )}">↓</a><button class="icon-button danger" type="button" title="Remove reference" aria-label="Remove ${escape(
        file.name.value
