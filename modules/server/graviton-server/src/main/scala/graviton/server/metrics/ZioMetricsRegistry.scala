  ...
  override def counterBy(name: String, delta: Long, tags: Map[String, String]): UIO[Unit] =
    if delta <= 0L then ZIO.unit
      ...
      countersRef
        ...
        .zipRight(tagged(Metric.counter(name), tags).incrementBy(delta))

  override def gauge(name: String, value: Double, tags: Map[String, String]): UIO[Unit] =
    gaugesRef
      .update(_.updated(MetricKey(name, tags), value))
      .zipRight(tagged(Metric.gauge(name), tags).set(value))

  override def histogram(name: String, value: Double, tags: Map[String, String]): UIO[Unit] =
    gaugesRef
      .update(_.updated(MetricKey(name, tags), value))
      .zipRight(tagged(Metric.histogram(name, ZioMetricsRegistry.DurationBoundaries), tags).update(value))
...
object ZioMetricsRegistry:
  ...
  val layer: ZLayer[PrometheusPublisher, Nothing, MetricsRegistry] =
    ZLayer.fromZIO {
      for
      ...
      yield ZioMetricsRegistry(counters, gauges, publisher)
    }
