  ...
  def apply(): HtmlElement = {
    ...
    def validateFile(file: dom.File): List[ValidationResult] = {
      ...
      if (file.size == 0) {
        ...
        validations += ValidationResult.Success(s"File size: ${formatBytes(file.size)}")
      }
        ...
        if (analyses.isEmpty) emptyNode
          ...
          div(
            ...
            div(
              ...
              div(cls := "stat-item",
                span(cls := "stat-label", "Total Files:"),
                span(cls := "stat-value", analyses.size.toString),
              ),
              div(cls := "stat-item",
                span(cls := "stat-label", "Total Chunks:"),
                span(cls := "stat-value", totalChunks.toString),
              ),
              div(cls := "stat-item",
                span(cls := "stat-label", "Unique Chunks:"),
                span(cls := "stat-value", uniqueChunks.toString),
              ),
              div(cls := "stat-item",
                span(cls := "stat-label", "Shared Chunks:"),
                span(cls := "stat-value", sharedChunks.toString),
              ),
              div(cls := "stat-item",
                span(cls := "stat-label", "Dedup Ratio:"),
                span(cls := "stat-value", f"${deduplicationRatio}%.3f"),
              ),
              div(cls := "stat-item",
                span(cls := "stat-label", "Space Savings:"),
                span(cls := "stat-value", f"${spaceSavings}%.1f%%"),
              ),
            ),
          )
        }
            ...
            div(
              ...
              div(
                ...
                div(cls := "stat-item",
                  span(cls := "stat-label", "Shared Chunks:"),
                  span(cls := "stat-value", 
                    (analysis.totalChunks - analysis.chunks.count(_.sharedWith.isEmpty)).toString),
                ),
              ),
            ),
            ...
            div(
              ...
              div(
                ...
                table(
                  ...
                  tbody(
                    analysis.chunks.take(100).map { chunk =>
                      tr(
                        cls := if (chunk.sharedWith.nonEmpty) "shared-chunk" else "",
                ...
                if (analysis.chunks.length > 100) {
                  div(cls := "truncation-notice",
                    s"Showing first 100 of ${analysis.chunks.length} chunks")
                } else emptyNode,
              ),
            ),
          )
