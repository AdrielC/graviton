...
object TransducerBench extends ZIOSpecDefault:
  ...
  override def spec: Spec[TestEnvironment, Any] =
    ...
    ) @@ TestAspect.sequential @@ ignore
    // Ignored under Scala 3.8+: inner assertions read `summary.digestHex`
    // and `summary.totalBytes` via kyo.Record's `selectDynamic` macro,
    // which regresses at 3.8 (Field-key HashMap miss). Reopen when kyo
    // ships 3.8 support.
