...
object StateSpec extends ZIOSpecDefault:
  ...
  def spec = suite("Named-Tuple State")(
    ...
    test("State composition in FreeScan preserves labels") {
      ...
      val scan1 = FreeScan.fromScan(Scan.stateful[Chunk, Chunk, Byte, Byte, S1](
        InitF.pure(rec.field("count", 0L) *: EmptyTuple),
        BiKleisli(cb => cb.map(b => ((rec.field("count", 1L) *: EmptyTuple), b))),
        _ => None
      ))
      
      val scan2 = FreeScan.fromScan(Scan.stateful[Chunk, Chunk, Byte, Byte, S2](
        InitF.pure(rec.field("hash", Array.empty[Byte]) *: EmptyTuple),
