...
object FreeScanV2Spec extends ZIOSpecDefault:
  ...
  // Ignored under Scala 3.8+: several tests fold their output into a
  // `kyo.Record` ("entries" / "total" etc.) and then read fields back out;
  // `kyo.Record.selectDynamic` regresses at 3.8 because the macro's stored
  // Field key hash differs from the READ-site Field key. Production code
  // does not touch these accessors. Reopen once kyo supports 3.8.
