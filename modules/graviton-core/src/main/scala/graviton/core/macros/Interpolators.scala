...
object Interpolators:
  ...
  private def bytesBlockImpl(scExpr: Expr[StringContext], argsExpr: Expr[Seq[Any]])(using Quotes): Expr[Block] =
    ...
    standardInterpolator(bytes => bytes.getBytes("US-ASCII").mkString, (arg: Any) => arg.toString, 
    argsExpr.asExprOf[List[Hasher.Digestable]].value.toSeq.flatMap(i =>
        i.flatMap(i => Hasher.systemDefault.toSeq.map(h => h.update(i).digest))
    ), scExpr.value.toList.flatMap(_.parts)) match
