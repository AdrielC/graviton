...
def get[A <: Rec, K <: String & Singleton](r: A, k: K): Get[A, K] =
  r match
    case r: NonEmptyTuple =>
      ...
      if (k == key) then value.asInstanceOf[Get[A, K]]
      else get[Tuple.Tail[A] & Rec, K](r.tail.asInstanceOf[Tuple.Tail[A] & Rec], k).asInstanceOf[Get[A, K]]
    case EmptyTuple       =>
...
object rec:
  ...
  def put[A <: Rec, K <: String & Singleton, V](a: A, k: K, v: V): Put[A, (K, V)] =
    a match
      ...
      case a: NonEmptyTuple =>
        ...
        if (k == key) then ((k, v) *: a.tail).asInstanceOf[Put[A, (K, V)]]
        else
          val h = a.head
  ...
  private def putRaw[A <: Rec](a: A, k: String, v: Any): A =
    a match
      ...
      case a: NonEmptyTuple =>
        val (key, value) = a.head.asInstanceOf[(String, Any)]
        if (k == key) then ((k, v) *: a.tail).asInstanceOf[A]
        else (a.head *: putRaw(a.tail.asInstanceOf[Rec], k, v)).asInstanceOf[A]
