...
object ByteConstraints:
  ...
  // CAS policy: persisted artifacts are non-empty.
  ...
  type ChunkCount      = Long :| numeric.Greater[0]
  ...
  def refineFileSize(value: Long): Either[String, FileSize] =
    if value < MinFileBytes then Left(s"File size must be at least $MinFileBytes byte(s), got $value")
  ...
  def refineChunkCount(value: Long): Either[String, ChunkCount] =
    if value < 1 then Left(s"Chunk count must be positive, got $value")
