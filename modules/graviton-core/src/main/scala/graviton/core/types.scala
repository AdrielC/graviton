...
object types:
    ...
    trait Specialized[
      ...
      self: Trait[Mn, Mx, Z, O] & RefinedType[Tpe, TCConstraint[Mn, Mx]] =>
        ...
        object SubT:
          ...
          val Max: Max   = self.applyUnsafe(mxV.value)
          val Min: Min   = self.applyUnsafe(mnV.value)
          val Zero: Zero = self.applyUnsafe(zV.value)
          val One: One   = self.applyUnsafe(oV.value)

          inline def applyUnsafe[t <: Tpe](t: t): SubT[t] = self.applyUnsafe(t)

          def either[t <: Tpe](t: t): Either[String, SubT[t]] =
            // no assume-cast: if refinement passed, we can safely treat it as SubT[t]
            self.either(t).map(_.asInstanceOf[SubT[t]])

          extension [t <: Tpe](x: SubT[t]) inline def value: t = x.asInstanceOf[t]

        export SubT.{ Max, Min, Zero, One }
      ...
      self =>
        ...
        given Constraint[T, numeric.GreaterEqual[Mn] & numeric.LessEqual[Mx]] =
          new Constraint[T, numeric.GreaterEqual[Mn] & numeric.LessEqual[Mx]]:
            inline def message: String = s"value must be between $Mn and $Mx"
  ...
  object FileSize extends SizeLongSubtype.Trait[0L, 1024L * 1024L * 1024L * 1024L, 0L, 1L] // 1 TiB
