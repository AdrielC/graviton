...
// Explicit `given JsonCodec[T]` on each RefinedSubtype works around the
// Scala 3.8 regression where `derives JsonCodec` can no longer pick up
// iron-zio-json's separate `JsonEncoder` / `JsonDecoder` givens for
// path-dependent `RefinedSubtype#T` members.
...
object BlockSize extends RefinedSubtype[Int, BlockSizeConstraint]:
  given JsonCodec[BlockSize] =
    JsonCodec(summon[JsonEncoder[BlockSize]], summon[JsonDecoder[BlockSize]])
...
object BlockIndex extends RefinedSubtype[Int, GreaterEqual[0]]:
  given JsonCodec[BlockIndex] =
    JsonCodec(summon[JsonEncoder[BlockIndex]], summon[JsonDecoder[BlockIndex]])
...
object ByteCount extends RefinedSubtype[Long, GreaterEqual[0L]]:
  given JsonCodec[ByteCount] =
    JsonCodec(summon[JsonEncoder[ByteCount]], summon[JsonDecoder[ByteCount]])
...
object HexDigest extends RefinedSubtype[String, HexLowerConstraint]:
  given JsonCodec[HexDigest] =
    JsonCodec(summon[JsonEncoder[HexDigest]], summon[JsonDecoder[HexDigest]])
...
object Sha256Hex extends RefinedSubtype[String, Sha256HexConstraint]:
  given JsonCodec[Sha256Hex] =
    JsonCodec(summon[JsonEncoder[Sha256Hex]], summon[JsonDecoder[Sha256Hex]])

/** Hash algorithm name constrained to known values. `Algo <: String`. */
type Algo = Algo.T
object Algo extends RefinedSubtype[String, AlgoConstraint]:
  given JsonCodec[Algo] =
    JsonCodec(summon[JsonEncoder[Algo]], summon[JsonDecoder[Algo]])
