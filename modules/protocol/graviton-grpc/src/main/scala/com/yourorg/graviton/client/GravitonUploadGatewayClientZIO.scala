    ...
    def fromFrames(frames: Chunk[ServerFrame]): IO[UploadGatewayError, UploadOutcome] =
      frames
        .foldLeftZIO(Accumulator()) { (state, frame) =>
          frame.kind match
            case ServerFrame.Kind.StartAck(value) =>
              ZIO.succeed(state.copy(startAck = Some(value)))
            ...
            case ServerFrame.Kind.Progress(value) =>
              ZIO.succeed(state.copy(progress = state.progress :+ value))

            case ServerFrame.Kind.Completed(value) =>
              ZIO.succeed(state.copy(completed = Some(value)))

            case ServerFrame.Kind.Event(value) =>
              ZIO.succeed(state.copy(events = state.events :+ value))
