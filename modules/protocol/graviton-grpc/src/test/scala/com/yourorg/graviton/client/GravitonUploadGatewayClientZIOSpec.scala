    ...
    suite("GravitonUploadGatewayClientZIO")(
      test("uploadFrames streams frames, enforces ack ordering, and returns completion") {
        for {
          ...
          gateway   = RecordingGateway(recorded,
                        Chunk(
                          ServerFrame(ServerFrame.Kind.StartAck(StartAck(sessionId = "sess-1", ttlSeconds = 60L, acceptedContentTypes = List("application/graviton-frame")))),
                          ...
                          ServerFrame(ServerFrame.Kind.Completed(Completed(sessionId = "sess-1", documentId = "doc-1", blobHash = "hash-123", objectContentType = "application/pdf", finalUrl = Some("https://example")))),
                        ))
          uploadSvc = new InMemoryUploadService
          client    = new GravitonUploadGatewayClientZIO(gateway, uploadSvc)
          frame1     = DataFrame(sessionId = "sess-1", sequence = 0L, offsetBytes = 0L, contentType = "application/graviton-frame", bytes = Chunk.fromArray("frame-0".getBytes), last = false)
          frame2     = DataFrame(sessionId = "sess-1", sequence = 1L, offsetBytes = 6L, contentType = "application/graviton-frame", bytes = Chunk.fromArray("frame-1".getBytes), last = true)
      ...
      test("uploadFrames fails fast on TTL expiry") {
        val responses = Chunk(
          ...
          ServerFrame(ServerFrame.Kind.Error(Error(code = Error.Code.SESSION_EXPIRED, message = "expired", details = Map("reason" -> "ttl")))),
        )
      ...
      test("uploadFrames surfaces metadata validation events") {
        val responses = Chunk(
          ...
          ServerFrame(ServerFrame.Kind.Event(Event(topic = "metadata.validation", message = "schema", attributes = Map("validation" -> "true")))),
          ServerFrame(ServerFrame.Kind.Completed(Completed(sessionId = "meta", documentId = "doc", blobHash = "hash", objectContentType = "application/pdf"))),
        )
        val client = new GravitonUploadGatewayClientZIO(RecordingGateway.drain(responses), new InMemoryUploadService)
        client
          .uploadFrames(
