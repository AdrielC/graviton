    ...
    suite("GravitonCatalogClientZIO")(
      test("search builds filters for hash, content type, and size range") {
        for {
          ...
          _      <- client.search(SearchFilters(hashes = Chunk("hash"), contentTypes = Chunk("application/pdf"), sizeRange = Some(0L -> 1024L))).runDrain
          stored <- ref.get
        } yield assertTrue(
      ...
      test("findDuplicates groups identical blobs") {
        val duplicates = DuplicateGroup(key = "sha", items = List(SearchResult(documentId = "doc", blobHash = "sha", contentType = "application/pdf", sizeBytes = 10L)))
        ...
        client.findDuplicates(FindDuplicatesRequest(hashPrefix = "sha")).runCollect.map(results => assertTrue(results.head.items.head.blobHash == "sha"))
      },
      test("export manifest and frames can be re-uploaded through frames path") {
        val manifest = ExportChunk(ExportChunk.Kind.Manifest(Manifest(contentType = "application/vnd.graviton.manifest+json", bytes = Chunk.fromArray("{}".getBytes))))
        val frame    = ExportChunk(ExportChunk.Kind.Frame(ExportFrame(contentType = "application/graviton-frame", bytes = Chunk.fromArray("block".getBytes))))
        val catalogStub = new CatalogClient:
          ...
          override def export(request: ExportRequest): ZStream[Any, Status, ExportChunk] = ZStream(manifest, frame)
          override def subscribe(request: SubscribeRequest): ZStream[Any, Status, CatalogEvent] = ZStream.empty
        for {
          recorded   <- Ref.make(Chunk.empty[ClientFrame])
          gatewayStub = RecordingGateway(recorded,
                          Chunk(
                            ...
                            ServerFrame(ServerFrame.Kind.Completed(Completed(sessionId = "sess", documentId = "doc", blobHash = "hash", objectContentType = "application/octet-stream"))),
                          ))
          ...
          exported <- catalogClient.export(ExportPlan(documentIds = Chunk("doc"))).runCollect
          _ <- gatewayClient.uploadFrames(
                 StartUpload(objectContentType = "application/octet-stream", metadata = List.empty),
                 ZStream.fromIterable(exported.collect { case ExportChunk(ExportChunk.Kind.Frame(f)) => Left(DataFrame(sessionId = "sess", sequence = 0L, offsetBytes = 0L, contentType = f.contentType, bytes = f.bytes, last = true)) }),
                 complete = Complete(sessionId = "sess"),
               )
          captured <- recorded.get
          } yield assertTrue(exported.length == 2, captured.nonEmpty)
      },
