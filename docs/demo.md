...
<script type="module">
  // Load the Scala.js compiled application
  // The main module is generated by Scala.js with the module name
  import('/graviton/js/main.js').catch(err => {
    console.error('Failed to load Graviton app:', err);
    document.getElementById('graviton-app').innerHTML = 
      '<div class="error-message">⚠️ Failed to load interactive demo. Make sure to build the frontend with: <code>sbt buildFrontend</code></div>';
  });
</script>
