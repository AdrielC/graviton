...
| Multi-tenant contracts | Authenticated HTTP and gRPC binding, durable cell-scoped policy, private manifests, forced PostgreSQL RLS, isolated or explicit shared domains, cluster-atomic retained quotas, Redis or Valkey HTTP request and delivered-egress contracts, bounded sharded local admission, cluster-wide transfer admission, immutable domain snapshots, scheduled cold-block scrub, a connection-coalescing maintenance barrier, and tenant laws | gRPC traffic limits remain process-local; exclusive collection drains a cell; target IdP acceptance, billing, physical faults, and target-scale capacity remain deployment work |
...
| Manifest authentication | Versioned signed Merkle B-tree root over blob identity, metadata, chunker, ordered block keys, and spans; verified before block fetch | Enable required mode, manage rotation keys, and keep HMAC or external signer material in a secret manager |
