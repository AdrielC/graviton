...
The scheduled `Production qualification` workflow builds the exact latest-release baseline and candidate images, starts the local two-node topology, runs `qualify-local-shardcake.sh`, retains corpus benchmark distributions, runs `qualify-rolling-upgrade.sh`, and finally runs `qualify-long-failure.sh` for ten minutes. A separate job starts three independently stoppable S3-compatible processes, writes 2+1 erasure shards, stops the preferred local process, proves remote byte-exact reconstruction, deletes another target's complete Docker volume, recreates it empty, waits for repair convergence, verifies the bytes again, and retains live Prometheus rules, metrics, dashboard JSON, and logs. Both artifacts are commit-addressed and retained for 90 days.
