...
| Multi-tenant contracts | Authenticated HTTP and gRPC binding, durable cell-scoped policy, private manifests, forced PostgreSQL RLS, isolated or explicit shared domains, cluster-atomic retained quotas, bounded sharded local admission, optional cluster-wide transfer admission, a connection-coalescing cell maintenance barrier, tenant laws, and domain-wide GC marks | Exclusive maintenance drains a cell; in-process faults are not physical power loss; global request and delivered-egress limits, target IdP acceptance, billing, and target-scale capacity remain deployment work |
...
The main product gaps are scheduled multi-tenant domain scrubbing, durable shared-domain membership snapshots, contractual global request and delivered-egress limiting at the authenticated edge, physical power-loss and disk-corruption injection, retained multi-tenant benchmark envelopes from multiple operator environments, and acceptance against each real IdP, ingress, Ceph cluster, Valkey topology, and storage provider. The repository now provides authenticated tenant binding, cluster-atomic retained quotas and active-transfer admission, pooled connection bounds, resumable uploads, durable repair progress and dead letters, automatic durability orchestration, in-process CrashLab laws, destructive single-target volume-loss evidence, mixed-version rollback qualification, and a sustained outage drill, but those cannot make every deployment topology equivalent. RocksDB remains a durable key-value module rather than an advertised blob backend.
