...
The scheduled `Production qualification` workflow builds the exact latest-release baseline and candidate images, starts the local two-node topology, runs `qualify-local-shardcake.sh`, retains corpus benchmark distributions, runs `qualify-rolling-upgrade.sh`, and finally runs `qualify-long-failure.sh` for ten minutes. Its complete JSON and logs are retained as one commit-addressed artifact for 90 days.
...
The main product gaps are abrupt power-loss and disk-fault injection, durable repair dead-letter state and restart cursor, coordinated provider snapshots, retained benchmark envelopes from multiple operator environments, and acceptance against each real IdP, ingress, and storage provider. The repository now provides resumable uploads, automatic replica orchestration, 90-day CI evidence, mixed-version rollback qualification, and a sustained outage drill, but those cannot make every deployment topology equivalent. RocksDB remains a durable key-value module rather than an advertised blob backend.
