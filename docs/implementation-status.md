...
| Manifest metadata, authentication, snapshots, and cold-block scrub | Current `main` | GVM5 and PostgreSQL persist a signed version-3 Merkle B-tree root; integrity, snapshot, repair, and maintenance suites are executable. This clean pre-1.0 line has no legacy manifest reader or backfill path. |
