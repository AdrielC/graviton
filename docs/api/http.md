...
```http
GET /api/v1/blobs?limit=100&cursor=<opaque-token>
```

`limit` must be between 1 and 1000. The response contains `blobs` and an optional `nextCursor`. The cursor is an Iron-refined opaque token generated by the active backend, not a blob ID or an offset. Pass it back unchanged. Filesystem uses bounded-memory selection, PostgreSQL uses keyset SQL, and S3 uses its native listing continuation. Inventory is derived from persisted manifests, not process counters.
