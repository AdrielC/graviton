# Archived migration playbook: 2025-11-06

> This is a historical planning snapshot. Branch names, module paths, dependency versions, commands, and proposed capabilities below may no longer exist. It is retained as a project record, not as current setup, architecture, or roadmap documentation. Use the [implementation status ledger](../implementation-status.md), [installation guide](../guide/installation.md), and repository build definition for current behavior.
...
- **Tasks**
  ...
  - Leverage Iron `RefinedSubtype` / `Subtype` machinery to derive constructors (`apply`, `either`, `option`, etc.) automatically—avoid bespoke smart-constructor code.
...
- **Artifacts to Port/Adapt**
  - `modules/core/src/main/scala/graviton/core/model/Block.scala` — block subtype, builders, chunkify helpers.
  - `modules/core/src/main/scala/graviton/bytes.scala` — stream wrappers (`Bytes`, `Blocks`).
  - `modules/core/src/main/scala/graviton/HashAlgorithm.scala`, `Hash.scala` (if needed for size/index aliases) to align naming.
  - Iron constraint definitions for byte limits (`ByteConstraints` in docs-deploy) to back smart constructors; reconcile with existing `graviton.core.types` aliases.
- **Integration Notes**
  - Merge iron constraints into the existing `graviton.core.types` module (currently defines `BlockSize`, `BlockIndex`, etc.); ensure no duplicate type aliases.
  - Extend `BinaryAttributes` tracked fields to accept new opaque wrappers while preserving `Tracked[A]` semantics and JSON codecs.
  - Provide interop helpers for legacy APIs that still expect `Chunk[Byte]` or raw numeric sizes; favor `zio.prelude` data types (`NonEmptyChunk`, `Validation`, `Newtype`, etc.) where applicable.
  - Introduce coherent package structure (`graviton.core.model`) inside `modules/graviton-core` to host byte constraints and block subtypes while keeping public API imports ergonomic.
- **Test Plan**
  - Unit: constructor success/failure cases (via Iron-provided helpers), conversions to/from primitives, interactions with `Tracked` updates, and `zio.prelude` data-type utilities.
  - Property (optional): ensure block size invariants hold across random chunk splits.
...
- **Tasks**
  ...
  - Maintain job logs (`docs/logs/`) and prune outdated planning references.
...
- **Tasks**
  ...
  - Final sweep of planning notes, leaving only active directives.
...
- Update this log after each phase with:
  ...
  - Next planned work.
- Weekly prune obsolete planning entries; reference this log for the authoritative plan.
...
Phase Execution Log
-------------------
- **2025-11-06 @ 05:54 UTC — Phase 0 (Baseline & Tooling Sync)**
  - Ran `TESTCONTAINERS=0 ./sbt scalafmtAll test`; all suites passed (ZStreamDecoder, RangeSet, RangeTree, FreeScan, protocol clients).
  - Installed VitePress dependencies (`npm install`) and built the docs site (`npm run docs:build`); build succeeded with `hocon` syntax highlight fallback warnings and expected bundle size notice.
  - Acceptance criteria met: repo clean, baseline tests + docs build green.
  - Next: kick off Phase 1 scaffolding (type foundations) while tracking docs warnings for later syntax highlighter tuning.
- **2025-11-06 @ 06:19 UTC — Phase 1 (Type Foundations) in progress**
  - Added `ByteConstraints` with Iron-refined aliases and safe/unsafe helpers for block, upload chunk, file size, chunk counts, and indices; avoided `RefinedSubtype` since iron 2.6.0 only ships `RefinedTypeOps`-style APIs.
  - Introduced opaque `UploadChunk` and `Block` wrappers with constructors, accessors, and a `BlockBuilder` helper; wired `BinaryAttributes` to use the new `FileSize`/`ChunkCount` types.
  - Updated `graviton.core.types` to re-export the refined aliases so downstream code keeps importing `types.BlockSize`/`types.BlockIndex` seamlessly; schemas now defer to `ByteConstraints.refine*` helpers.
  - Added `ByteConstraintsSpec` and `BlockSpec` to cover happy/invalid paths and chunkify behaviour.
  - Full suite green via `TESTCONTAINERS=0 ./sbt test`.
  - Documentation: expanded `docs/ingest/chunking.md` with guidance on the new opaque blocks/attributes flow and taught VitePress to remap ` ```hocon` fences to ` ```ini` so `npm run docs:build` no longer emits language warnings.
  - TODO: document the new helpers, decide whether to expose additional interoperability shims (e.g., `ChunkIndex` smart constructors), and extend BinaryAttributes examples to mention the refined types.
