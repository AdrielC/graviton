...
The packaged CLI, HTTP gateway, gRPC service, and Shardcake owner streams use these runtime services today. Each backend contributes to a named `TransferFootprint`; one hierarchical `TransferBudget` then enforces process bytes, tenant bytes and concurrency, and backend concurrency. When replica targets are configured, the server starts one supervised bounded repair worker whose cursor and unresolved failures survive restart through `FsRepairJournal` or `PgRepairJournal`.
...
Backend adapters obtain a `MetricsRegistry` and emit events through bounded-cardinality tags. PostgreSQL publishes pool state, while the AWS SDK metric publisher records S3 API calls and retry counts. The runtime module stays vendor-neutral by restricting itself to key naming conventions.
