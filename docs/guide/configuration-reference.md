...
The snapshot and SSE endpoints require `observability.read` when security is enabled. They aggregate fixed operational dimensions and never include payloads, object names, content IDs, tenant IDs, or the raw metrics registry. See [Operator Control Plane](../ops/control-plane.md).
