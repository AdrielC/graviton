...
The **`graviton-cli`** module provides ingest, stat, get, and verify commands. Run it via SBT (there is no separate release binary in-repo yet):

```bash
GRAVITON_DATA_DIR=/tmp/graviton-data ./sbt "cli/run ingest /path/to/file"
GRAVITON_DATA_DIR=/tmp/graviton-data ./sbt "cli/run stat <blobDigestHex>"
GRAVITON_DATA_DIR=/tmp/graviton-data ./sbt "cli/run get <blobDigestHex> /tmp/out.bin"
GRAVITON_DATA_DIR=/tmp/graviton-data ./sbt "cli/run verify <blobDigestHex>"
```

The CLI uses a **filesystem** `BlockStore` under `GRAVITON_DATA_DIR` and an **in-memory** manifest repository per process — `stat` / `get` / `verify` only see blobs from the **same JVM run** as `ingest`. For durable manifests across runs, use **`graviton-server`** with Postgres (see below) or embed `BlobStore` in your own app.

You may also see `cli/run`-style hints in the Scala.js demo UI; those should match this SBT invocation.

## Without the CLI

You can still:

- Run the **HTTP server** (`./sbt "server/run"`) and use **curl** (see below).
- Call **`Graviton`**, **`BlobStore`**, and **`BlockStore`** directly from Scala.
