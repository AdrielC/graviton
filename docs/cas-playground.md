...
Drop two or more real files into the byte atlas. The Scala.js analyzer streams each browser `File`, chooses a chunker from the bytes, hashes every block, and aligns equal content IDs across files. Cyan ranges are reusable content. Selecting a range shows every exact match.

<PipelinePlayground />

## What runs here

The playground is a local analysis tool, not a simulated server:
...
- Cross-file reuse is counted only when block content IDs match exactly. The estimate does not claim filesystem allocation, compression, or persisted savings.

The default target is 256 KiB. Choose 16 KiB, 64 KiB, 256 KiB, or 1 MiB and reanalyze the same files. A block is never larger than 4 MiB, the result manifest is capped at 20,000 blocks per file, and the UI analyzes at most two files concurrently.
...
**Create and compare** produces two local outputs through the same ZIO PDF encoder: an unchanged canonical source and a font-remapped variant. It switches the atlas to the 16 KiB automatic profile and analyzes both outputs, so exact reuse measures the local font edit instead of unrelated differences between the uploaded PDF's original serializer and ZIO PDF's encoder. Both outputs can be downloaded.
