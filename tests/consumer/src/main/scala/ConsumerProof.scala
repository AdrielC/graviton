...
object ConsumerProof extends ZIOAppDefault:
  override def run: Task[Unit] =
    ...
    for
      ...
      _        <- Console.printLine(
                    s"external-consumer-proof key=${protoKey.value} bytes=${restored.length} media=${mediaType.fullType} max=$maxSize kv=${kvKey.value}:${kvValue.length} token-bytes=${token.value.length} health=${health.status}"
                  )
    yield ()
